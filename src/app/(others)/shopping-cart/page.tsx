...
const ShoppingCart: React.FC = () => {
  ...
  return (
    ...
    <div className="bg-white font-sans max-w-[1320px] mx-auto">
          ...
          <tbody>
            {cartItems.map((item, index) => (
              <tr key={index} className="border-b">
                <td className="p-4 flex items-center flex-col md:flex-row">
                  <Image src={item.image} alt={item.name}
                  width={16} height={16} className="w-16 h-16 object-cover rounded mr-2 md:mr-4" />
                  <span>{item.name}</span>
                </td>
